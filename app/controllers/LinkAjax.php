...
class LinkAjax extends Controller {
    ...
    private function create_biolink_other__($subtype) {
    ...
    private function create_biolink_other($subtype) {
        ...
        if($link_id = Database::simple_get('link_id', 'links', ['user_id' => 1, 'biolink_id' => $_POST['link_id'], 'type' => 'biolink', 'subtype' => 'vimeo'])) {
            // die();
            Response::json("Vimeo URL is already existed!", 'error');
        }

        $location_url = $_POST['location_url'] . '/' . $startTime . '/' . ($startTime + $duration);
        $user_id = 1;
        ...
        $stmt = Database::$database->prepare("INSERT INTO `links` (`project_id`, `biolink_id`, `user_id`, `type`, `subtype`, `url`, `location_url`, `settings`, `order`, `date`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param('ssssssssss', $project_id, $_POST['link_id'], $user_id, $type, $subtype, $url, $location_url, $settings, $new_order, \Altum\Date::$date);
        $stmt->execute();
        $stmt->close();
